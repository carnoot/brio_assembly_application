...
pcl::PointCloud<pcl::PointXYZ>::Ptr DepthImageProjection::project(const cv::Mat &depth_image_in)
{
  cv::Mat depth_image;
  ...
  register const float constant = 1.0f / 525,
//  register const float constant = 1.0f / 570,
      bad_point = std::numeric_limits<float>::quiet_NaN();
  register const int centerX = (cloud->width >> 1), centerY = (cloud->height >> 1);

#pragma omp parallel for
  for (int y = 0; y < depth_image.rows; ++y)
  {
    register pcl::PointXYZ *pPt = &cloud->points[y * depth_image.cols];
    register const float *pDepth = depth_image.ptr<float>(y), v = (y - centerY) * constant;

    for (register int u = -centerX; u < centerX; ++u, ++pPt, ++pDepth)
    {
...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr DepthImageProjection::project(const cv::Mat &depth_image_in,
